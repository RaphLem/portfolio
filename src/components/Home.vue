<template>
  ...
  <span id="welcome">
    ...
    <div id="about">
      <h2>About me</h2>
      <p id="presentation">Hello, my name is <strong>Raphaël Lemoine</strong>. I'm a Front end web developer focused on
        integration of
        design and animation with HTML/CSS/JS. I work with some framework like VueJs too and i'm doing a bit of back-end
        too. I am passionate about technologies and japanese culture.
      </p>
    </div>
    <div id="portrait">
      <img src="static/img/Photo-CV-MMI-4.png" alt="portrait">
      <figcaption>Shot by <a target="_blank" href="https://www.instagram.com/phasme.raw/">Joris Lin</a>
      </figcaption>
    </div>
    <div id="skills">
      <h3>
      ...
      <ul id="list-skills">
        <li>Realization of web sites for projects or for individuals</li>
        <li>Mastery of development languages ( HTML/CSS/JS )</li>
        <li>Use of framework ( VueJS )</li>
        <li>Implementation of a headless back end ( WordPress, Firebase, Json)</li>
        <li>Creation of mobile application with Ionic</li>
        <li>Creation of various graphic design</li>
        <li>Good level with Adobe suite (Xd, Illustrator, After Effect)</li>
      </ul>
    </div>
    <a href="../../static/CV_LEMOINE_Raphaël.pdf" download id="resume">
      Download my resume
    </a>
    <div id="contact">
      ...
      <ul id="list-contact">
        <li>
          <svg id="mail" fill="#000000" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="40px"
               height="40px">
            <path
          ...
          <a href="mailto:raphael-lemoine28@laposte.net" target="_blank">Mail</a></li>
        <li>
          <svg id="github" fill="#000000" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="40px"
               height="40px">
            <path
          ...
          <a href="https://github.com/RaphLem" target="_blank">Github</a></li>
        <li>
          <svg id="linkedin" fill="#000000" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="40px"
               height="40px">
            <path
          ...
          <a href="https://fr.linkedin.com/in/raphaël-lemoine-83411a200" target="_blank">LinkedIn</a></li>
      </ul>
    </div>
  </div>
</template>
...
export default {
  name: "Accueil",
  mounted() {
    const skillUl = document.querySelector('#list-skills')
    const skillLi = document.querySelectorAll('#list-skills li');
    const skillActive = document.querySelectorAll('#list-skills li.active');

    const contactUl = document.querySelector('#list-contact')
    const contactLi = document.querySelectorAll('#list-contact li');

    window.addEventListener('scroll', () => {

      const {scrollTop, clientHeight} = document.documentElement;

      const topElementToTopSkill = skillUl.getBoundingClientRect().top;

      if (scrollTop > (scrollTop + topElementToTopSkill).toFixed() - clientHeight * 0.60) {
        skillLi.forEach(function (s, i) {
          s.classList.add('active');
        })
      }

      const topElementToTopContact = contactUl.getBoundingClientRect().top;

      if (scrollTop > (scrollTop + topElementToTopContact).toFixed() - clientHeight * 0.80) {
        contactLi.forEach(function (s, i) {
          s.classList.add('active');
        })
      }
